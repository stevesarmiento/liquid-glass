...
export const LensDeform = styled.span`
  ...
  z-index: 2;
  /* Rendered (active-swollen) box; falls back to the resting size. The
     track's insets stay on --lgds-slider-lens-width (RESTING) so the visible
     track does not shrink when the thumb swells. */
  width: var(--lgds-slider-lens-box-width, var(--lgds-slider-lens-width));
  height: var(--lgds-slider-lens-box-height, var(--lgds-slider-lens-height));
  overflow: visible;
  pointer-events: none;
`;
