...
pub fn generate_displacement_map(params: &LensParams) -> DisplacementMap {
    ...
    let edge_range = 3.0_f32.max(1.0);
    let glow_threshold = (1.0 - 0.62) * std::f32::consts::SQRT_2;
    let glow_range = 0.62 * std::f32::consts::SQRT_2;
    let spec_rotation = 45.0_f32.to_radians();
    ...
    for py in 0..half_size {
        ...
        for px in 0..half_size {
            ...
            if outer >= 0.0 {
                write_symmetric_pixels(&mut rgba, size, px, py, 128, 128, 128, true);
                continue;
            }
            ...
            if params.glow > 0.0 {
                ...
                spec += params.glow * t.powf(1.5) * falloff;
            }
            if params.edge > 0.0 {
                ...
                spec += params.edge * edge_mask * highlight_axis.powf(1.2);
            }
...
fn compute_dome_constants(depth: f32, half_w: f32, half_h: f32) -> DomeConstants {
    let safe_depth = depth.max(0.01).min(half_w.min(half_h) - 1.0);
...
fn write_symmetric_pixels(
    ...
    let px_r = size - 1 - px;
    let py_b = size - 1 - py;
    write_pixel(rgba, size, px, py, r, g, b);
    if px_r != px {
        write_pixel(rgba, size, px_r, py, if neutral { r } else { 255 - r }, g, b);
    }
    if py_b != py {
        write_pixel(rgba, size, px, py_b, r, if neutral { g } else { 255 - g }, b);
    }
    if px_r != px && py_b != py {
        write_pixel(
            rgba,
